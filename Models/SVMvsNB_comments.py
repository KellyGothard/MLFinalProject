...
def get_data_rawposts(path1, path2):
    banned_df = pd.read_csv(path1).sample(frac = 0.01)
    notbanned_df = pd.read_csv(path2).sample(frac = 0.01)
    
    banned_df = banned_df[banned_df['body'].notna()]
    notbanned_df = notbanned_df[notbanned_df['body'].notna()]
    
    STEM = WordNetLemmatizer()
    
    sr_df = banned_df.append(notbanned_df)
...
def main():
    ...
    df, rank_df = get_data_rawposts(path1, path2)
#    plots(rank_df)
#    print(len(df))
#    print(df.groupby('banned').count())
#        
#    # Vectorize words
#    corpus = df['body']
...
#    plt.savefig('../images/posts_clf_barplot.png')

    plots.words_per_row(df,'Comment')
